...
func apply_enemy_stats():
	if Global.enemy_data.has(Global.level):
		...
		for enemy in enemy_list:
			if enemy.has("name") and enemy["name"] == "Skeleton":
				health = enemy["health"]
				max_health = health
				print("Goblin stats: Health =", health)
				return
		print("Skeleton stats not found in enemy list for level ", Global.level)
	else:
...
func _physics_process(delta):
	...
	if player_chase:
		...
		$AnimatedSprite2D.play("skeleton_walk")
		...
		$AnimatedSprite2D.play("skeletons_idle")
