	...
	if Global.enemy_data.has(Global.level):
		...
		for enemy in enemy_list:
			if enemy.has("name") and enemy["name"] == "Goblin":
				health = enemy["health"]
				max_health = health
				print("Mushroom stats: Health =", health)
				return
		print("Mushroom stats not found in enemy list for level ", Global.level)
	else:
...
func _on_enemy_2_hitbox_body_entered(body: Node2D) -> void:
	if body.has_method("player"):
		player_in_attack_zone = true


func _on_enemy_2_hitbox_body_exited(body: Node2D) -> void:
	if body.has_method("player"):
...
func deal_with_damage():
	...
	if player_in_attack_zone and (Global.player_current_attack1 or Global.player_current_attack2):
		if can_take_damage:
			...
			print("Mushroom enemy health = ", health)

			if health <= 0:
				print("Mushroon enemy dead")
